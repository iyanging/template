...
def _getattr(obj: Union[dict, BaseModel], item):
...
class Query(BaseQuery):
    ...
    def decode(self, obj_in: Union[dict, BaseModel], exclude_unset: bool = False) -> Any:
        if isinstance(obj_in, dict):
            values = obj_in
        elif isinstance(obj_in, BaseModel):
            values = obj_in.dict(exclude_unset=exclude_unset)
        else:
            raise RuntimeError('Query.decode only accept dict or pydantic Model.')
    ...
    async def update_many(
        self,
        obj_in: List[Union[dict, BaseModel]],
        update_fields: List[str],
        where_fields: Optional[List[str]] = None,
    ) -> Any:
        if not obj_in:
            return None
        values = []
        ...
        for o in obj_in:
            value = self.decode(o, exclude_unset=True)
            if value:
                values.append(value)
        ...
        # bind where param
        if not where_fields:
            where_fields = [self.primary_key.name]
        for where_field in where_fields:
            stmt = stmt.where(self.table.c[where_field] == bindparam(where_field))
        # bind values param
        stmt = stmt.values({field: bindparam(field) for field in update_fields})

        return await db.execute_many(str(stmt), values)

    async def update(self, obj_in: Union[dict, BaseModel], **_) -> Any:
        values = self.decode(obj_in, exclude_unset=True)
        if not values:
            return None
    ...
    async def exists(self):
        stmt = BaseQuery(super().exists()).statement
    ...
    async def update_m2m_more(
            ...
            if old:
                await db.execute(
                    self.table.delete().where(
                        self.table.c[self.pk].in_([_getattr(o, self.pk) for o in old])
                    )
                )
            return None
        ...
        for obj in old:
            old_map[f'{_getattr(obj, fk1)}:{_getattr(obj, fk2)}'].append(_getattr(obj, pk_name))
        new_map = {f'{_getattr(obj, fk1)}:{_getattr(obj, fk2)}': obj for obj in new}
        ...
        if (
            ...
            for key in update_keys:  # noqa
                obj = new_map[key]
                stmt = (
                    self.table.update()
                    .where(self.table.c[fk1] == _getattr(obj, fk1))
                    .where(self.table.c[fk2] == _getattr(obj, fk2))
                )
